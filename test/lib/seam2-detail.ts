...
//
//   node test/lib/seam2-detail.ts <driver> < a session-state message
//
...
// so the assertions live in test/driver-detail.test.sh.

import type { SessionStateMessage } from '../../domain/index.ts';
import { driverDetail } from '../../web/driver-detail.ts';
...
process.stdout.write(`${driverDetail(message.state, driver === undefined ? undefined : Number(driver))}\n`);
